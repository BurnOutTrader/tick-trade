use std::sync::Arc;
use crate::reality_models::inbuilt::defaults::{DefaultFeeModel, DefaultFill, DefaultFuturesBP, DefaultSettle, DefaultSlippage, DefaultVol};
...
impl ModelCatalog {
    pub fn with_default(default: ModelSet) -> Self {
        Self { default_set: default, ..Default::default() }
    }
    ...
    /// 3) (kind, None, None)
    /// 4) default_set
    pub fn select(&self, security_type: SecurityType, exchange: Option<Exchange>, product_root: Option<&str>) -> &ModelSet {
        if let Some(root) = product_root {
            let key = ModelKey { security_type, exchange, product_root: Some(root.to_string()) };
            if let Some(ms) = self.by_key.get(&key) { return ms; }
        }
        if let Some(ms) = self.by_key.get(&ModelKey { security_type, exchange, product_root: None }) {
            return ms;
        }
        if let Some(ms) = self.by_kind.get(&security_type) { return ms; }
        &self.default_set
    }
}