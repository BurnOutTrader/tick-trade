...
use crate::apis::rithmic::rithmic_proto_objects::rti::Reject;
...
lazy_static! {
    pub static ref QUANTITY_LONG: DashMap<Account, DashMap<Symbol , Decimal>>= DashMap::new();
    pub static ref QUANTITY_SHORT: DashMap<Account, DashMap<Symbol , Decimal>> = DashMap::new();
}
    ...
    match (msg_buy_qty, msg_sell_qty) {
        (Some(buy_quantity), None) if buy_quantity > 0 => {
            QUANTITY_LONG
                ...
                .insert(symbol.clone(), Volume::from_i32(buy_quantity).unwrap());
            QUANTITY_SHORT
                .entry(account_id.clone())
                .or_insert_with(DashMap::new)
                .remove(symbol);
        ...
        (None, Some(sell_qty)) if sell_qty > 0 => {
            QUANTITY_SHORT
                .entry(account_id.clone())
                .or_insert_with(DashMap::new)
                .insert(symbol.clone(), Volume::from_i32(sell_qty).unwrap());
            QUANTITY_LONG
                .entry(account_id.clone())
                .or_insert_with(DashMap::new)
                .remove(symbol);
        ...
        (Some(buy_quantity), Some(sell_qty)) => {
            if buy_quantity > 0 && sell_qty <= 0 {
                ...
                QUANTITY_SHORT
                    .entry(account_id.clone())
                    .or_insert_with(DashMap::new)
                    .insert(symbol.clone(), Volume::from_i32(sell_qty.abs()).unwrap());
                QUANTITY_LONG
                    .entry(account_id.clone())
                    .or_insert_with(DashMap::new)
                    .remove(symbol);
