...
impl ReplayCoordinator {
    ...
    pub fn add_ticks(&mut self, tag: impl Into<String>, rows: Vec<Tick>) {
        self.sources.push(Box::new(SymbolKindReplayer::new_ticks(tag.into(), rows)));
    }
    pub fn add_candles(&mut self, tag: impl Into<String>, rows: Vec<Candle>) {
        self.sources.push(Box::new(SymbolKindReplayer::new_candles(tag.into(), rows)));
    }
    pub fn add_bbo(&mut self, tag: impl Into<String>, rows: Vec<Bbo>) {
        self.sources.push(Box::new(SymbolKindReplayer::new_bbo(tag.into(), rows)));
    }
    pub fn add_books(&mut self, tag: impl Into<String>, rows: Vec<OrderBook>) {
        self.sources.push(Box::new(SymbolKindReplayer::new_books(tag.into(), rows)));
    }
