...
pub fn earliest_ts(conn: &Connection, parquet_glob: &str, ts_col: &str) -> Result<Option<DateTime<Utc>>> {
...
pub fn latest_ts(conn: &Connection, parquet_glob: &str, ts_col: &str) -> Result<Option<DateTime<Utc>>> {
...
pub fn min_max_ts(conn: &Connection, parquet_glob: &str, ts_col: &str) -> Result<(Option<DateTime<Utc>>, Option<DateTime<Utc>>)> {
    let sql = format!("SELECT MIN({0}) AS min_ts, MAX({0}) AS max_ts FROM read_parquet(?);", ts_col);
    let mut stmt = conn.prepare(&sql)?;
    let mut rows = stmt.query(params![parquet_glob])?;
    if let Some(row) = rows.next()? {
