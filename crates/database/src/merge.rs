...
impl<'a, T: TimeKey + ?Sized> TimeKey for &'a T {
    #[inline] fn time_ns(&self) -> i128 { (**self).time_ns() }
    #[inline] fn tie_key(&self) -> (u32, &str, &str, &str) { (**self).tie_key() }
    #[inline] fn time_key(&self) -> (i128, u32) { (**self).time_key() }
}
...
impl<T: TimeKey> PartialEq for MergeNode<T> {
    ...
    fn eq(&self, other: &Self) -> bool {
        self.item.time_key() == other.item.time_key()
            && self.item.tie_key() == other.item.tie_key()
    }
}
...
impl<'a, T: TimeKey> KMerge<'a, T> {
    pub fn new(sorted_slices: Vec<&'a [T]>) -> Self {
        ...
        for (i, s) in sorted_slices.iter().enumerate() {
            if let Some(first) = s.first() {
                heap.push(Reverse(MergeNode { src: i, idx: 0, item: first }));
            }
        }
        Self { sources: sorted_slices, heap }
    }
    ...
    pub fn next(&mut self) -> Option<&'a T> {
        ...
        if let Some(next_item) = self.sources[src].get(next_idx) {
            self.heap.push(Reverse(MergeNode { src, idx: next_idx, item: next_item }));
        }
        Some(item)
    }
